...
$DIFF $DIFFFLAGS $file".colorizar.c.bmp" $TESTIMGCATEDRA/"lena.colorizar.catedra.bmp" 5
if [ $? != "0" ]; then
	OKDIFF=0
...
$DIFF $DIFFFLAGS $file".colorizar.asm.bmp" $TESTIMGCATEDRA/"lena.colorizar.catedra.bmp" 5
if [ $? != "0" ]; then
	OKDIFF=0
	echo "Diferencias en filtro: colorizar"
fi
...
$DIFF $DIFFFLAGS $file".umbralizar.min-64.max-128.q-16.c.bmp" $TESTIMGCATEDRA/"lena.umbralizar.catedra.bmp" 0
if [ $? != "0" ]; then
	OKDIFF=0
...
$DIFF $DIFFFLAGS $file".umbralizar.min-64.max-128.q-16.asm.bmp" $TESTIMGCATEDRA/"lena.umbralizar.catedra.bmp" 0
if [ $? != "0" ]; then
	OKDIFF=0
	echo "Diferencias en filtro: umbralizar"
fi
...
$DIFF $DIFFFLAGS $file".waves.x_scale-8.00.y_scale-8.00.g_scale-32.00.c.bmp" $TESTIMGCATEDRA/"lena.waves.catedra.bmp" 0
if [ $? != "0" ]; then
	OKDIFF=0
...
$DIFF $DIFFFLAGS $file".waves.x_scale-8.00.y_scale-8.00.g_scale-32.00.asm.bmp" $TESTIMGCATEDRA/"lena.waves.catedra.bmp" 0
if [ $? != "0" ]; then
	OKDIFF=0
	echo "Diferencias en filtro: waves"
fi
